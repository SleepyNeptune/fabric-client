    ...
    always {
      ...
      script {
        ...
        withCredentials([string(credentialsId: 'discord-webhook', variable: 'discordWebhook')]) {
            discordSend thumbnail: "http://wnuke.dev/radiation-symbol.png", successful: currentBuild.resultIsBetterOrEqualTo('SUCCESS'), description: "${msg}", link: env.BUILD_URL, title: "toastclient-fabric:${env.BRANCH_NAME} #${BUILD_NUMBER}", webhookURL: "${discordWebhook}"
        }
      }
    }
  }
